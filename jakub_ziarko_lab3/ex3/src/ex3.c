...
void run_program(int file_desc, char * buffor, int buffer_size, char * instructions_buf, char * arg, char ** array, char * path_to_program, int env, int max_cpu, int max_mem) {
    ...
    while (j != -1) {
        ...
        if ( j != -1) {
            ...
            child_pid = fork();
            if (child_pid == 0) {
                rl.rlim_max = (rlim_t) max_cpu;
                rl.rlim_cur = (rlim_t) max_cpu;
                setrlimit(RLIMIT_CPU, &rl);
                rl.rlim_max = (rlim_t) max_mem;
                rl.rlim_cur = (rlim_t) max_mem;
...
int main(int argc, char *argv[]) {
    ...
    int env_variable = 0, max_mem, max_cpu;
    ...
    printf("%s", "W pierwszym argumencie podaj ścieżkę do pliku z którego program będzie czytać komendy,\n "
            "w drugim cpu, w trzecim memory, w czwartym czy chcesz użyć zmiennej środowiskowej podając 1 jeśli tak, 0 jeśli nie.\n\n");

    file_name = argv[1];
    max_cpu = atoi(argv[2]);
    max_mem = atoi(argv[3]);
    env_variable = atoi(argv[4]);

    file_desc = create_open_file_sys(file_name);
    run_program(file_desc, buffor, buffor_size, instructions_buf, arg, array, path_to_program, env_variable, max_cpu, max_mem);

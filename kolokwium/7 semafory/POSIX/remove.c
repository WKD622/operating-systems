...
int main(){
	...
	if (SEMAPHORE == SEM_FAILED){
        printf("remove: Opening semaphore failed!\n");
        return -1;
    }


	// otwieranie współdzielonej pamięci
	int shmID = shm_open("/shm", O_RDWR, 0666);
    if (shmID == -1) printf("remove: Opening shared memory failed!");
	...
	while(1) {
 	...
 	if (sem_close(SEMAPHORE) == -1) printf("remove: Closing semaphore failed!\n");
	return 1;
}
